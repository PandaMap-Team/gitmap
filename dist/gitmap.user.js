(function() {
    ...
    const FTL_URL = 'https://raw.githubusercontent.com/PandaMap-Team/gitmap/main/locales/ru.ftl';
    const LOG_PREFIX = '[GitMap]';
    ...
    class GitHubLocalizer {
        ...
        /**
         * Локализация вкладок репозитория (верхняя полоска Code / Issues / Actions / Security / Insights)
         */
        localizeRepoTabs() {
            ...
            // Верхняя линия вкладок
            const tabSpans = repoNav.querySelectorAll(
                '.UnderlineNav-item span[data-content]'
            );
            ...
            tabSpans.forEach(span => {
                ...
                // и обновить data-content, чтобы GitHub не откатывал назад
            ...
            // Пункты в «Additional navigation options» (ActionList в overflow-меню)
        ...
        localizeRepoMetadataDialog() {
            ...
            if (descInput) {
                this.localizeByText(
                    ...
                    'short-description'
                ); // эта строка не через placeholder, а через парсер, но проще так:
            ...
            dialog.querySelectorAll('label[for^="repo_sections_"]').forEach(label => {
                ...
                if (t) {
                    textNode.textContent = ' ' + t; // пробел перед текстом, т.к. после input идёт текст
                }
            });

            // Кнопки Save changes / Cancel
